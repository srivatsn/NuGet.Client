...
namespace NuGet.Packaging.Signing
{
    internal static class NativeUtilities
    {
        ...
        internal static SignedCms NativeCounterSign(CmsSigner cmsSigner, byte[] data, CngKey privateKey)
        {
            using (var hb = new HeapBlockRetainer())
            {
                byte[] encodedData;
                ...
                var hMsg = NativeMethods.CryptMsgOpenToDecode(
                    CMSG_ENCODING.Any,
                    dwFlags: 0,
                    dwMsgType: 0,
                ...
                ThrowIfFailed(NativeMethods.CryptMsgUpdate(
                    hMsg, data, (uint)data.Length, fFinal: true));

                ThrowIfFailed(NativeMethods.CryptMsgCountersign(
                    hMsg,
                    dwIndex: 0,
                    cCountersigners: 1,
                ...
                ThrowIfFailed(NativeMethods.CryptMsgGetParam(
                    hMsg,
                    CMSG_GETPARAM_TYPE.CMSG_ENCODED_MESSAGE,
                    dwIndex: 0,
                    pvData: null,
                    pcbData: ref valueLength));
